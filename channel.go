...
func (p *policyEngine) getChanInfo(ctx psbtContext) (*chanInfo, error) {
	...
	acctKey, ok := p.accounts[[3]uint32{
		...
		p.r.keyRing.Coin() + hdkeychain.HardenedKeyStart,   // Coin type.
		7 + hdkeychain.HardenedKeyStart,                    // Encryption.
	}]
	if !ok {
		return nil, fmt.Errorf("couldn't get encryption base key")
	}

	branchKey, err := acctKey.DeriveNonStandard(0) // Always external.
	if err != nil {
		return nil, err
	}

	idxKey, err := branchKey.DeriveNonStandard(0) // Always 0 for now.
	if err != nil {
		return nil, err
	}
	...
	backup, err := os.ReadFile(p.r.cfg.ChanBackup)
	if err != nil {
		return nil, err
	}
		...
		if channel.chanPoint == chanPoint {
			...
			for _, el := range toDerive {
				actKey, ok := p.accounts[[3]uint32{
					...
					p.r.keyRing.Coin() + hdkeychain.HardenedKeyStart,
				...
				branchKey, err := actKey.DeriveNonStandard(0)
				if err != nil {
					return nil, err
				}

				idxKey, err := branchKey.DeriveNonStandard(
					el.Index,
				)
				if err != nil {
					return nil, err
...
func (c *chanInfo) enforcePolicy(ctx psbtContext) (interface{}, error) {
	...
	logger := signerLog.With("chan", c.chanPoint)
